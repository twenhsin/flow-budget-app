    ...
    messages: [
      {
        ...
        content: [
          {
            type: 'text',
            text: `這是一張台灣的收據，請嚴格依照以下規則辨識：

【辨識規則】
1. 品項名稱必須與收據上印刷的文字完全一致，不可自行翻譯、簡化、改寫或替換
2. 金額以收據上的數字為準，不可推算或估計
3. 若收據文字模糊無法辨識，該欄位回傳空字串 ""，不可猜測
4. 不可合併或拆分收據上的品項，每行商品各自獨立回傳
5. 排除非商品行：合計、小計、現金、找零、稅金、服務費、發票號碼等不得回傳

【輸出格式】
回傳純 JSON，不加任何說明文字或 markdown：
{"items": [{"name": "收據原文品項名稱", "amount": 金額數字, "category": "類別"}]}

類別只能從以下選擇：餐飲、飲料、點心、居住、休閒、交通、通訊、其他
