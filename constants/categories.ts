...
// SVG paths for the 20 Lucide icons used in suggest-category
export const LUCIDE_PATHS: Record<string, string> = {
  ...
  Car: '<path d="M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v9a2 2 0 0 1-2 2h-2"/><circle cx="8" cy="17" r="2"/><circle cx="16" cy="17" r="2"/><polyline points="8 15 3 15 3 10 14 10 14 15 8 15"/>',
  ShoppingBasket: '<path d="m5 11 4-7"/><path d="m19 11-4-7"/><path d="M2 11h20"/><path d="m3.5 11 1.6 7.4a2 2 0 0 0 2 1.6h9.8a2 2 0 0 0 2-1.6l1.7-7.4"/><path d="m9 11 1 9"/><path d="M4.5 15.5h15"/><path d="m15 11-1 9"/>',
  ...
  Gamepad2: '<line x1="6" x2="10" y1="11" y2="11"/><line x1="8" x2="8" y1="9" y2="13"/><line x1="15" x2="17" y1="12" y2="12"/><path d="M20 11H4a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-4a2 2 0 0 0-2-2z"/>',
...
export const CATEGORIES: CategoryDef[] = [
  {
    name: '餐飲',
    color: '#E8A149',
    path: '<path d="m16 2-2.3 2.3a3 3 0 0 0 0 4.2l1.8 1.8a3 3 0 0 0 4.2 0L22 8"/><path d="M15 15 3.3 3.3a4.2 4.2 0 0 0 0 6l7.3 7.3c.7.7 2 .7 2.8 0L15 15Zm0 0 7 7"/><path d="m2.1 21.8 6.4-6.3"/><path d="m19 5-7 7"/>',
  },
  {
    name: '飲料',
    color: '#6A93CD',
    path: '<path d="M17 8h1a4 4 0 1 1 0 8h-1"/><path d="M3 8h14v9a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4Z"/>',
  },
  {
    name: '點心',
    color: '#D97F81',
    path: '<path d="M12 2a9 9 0 0 1 9 9c0 3.18-2.09 6.36-4 8H7c-1.91-1.64-4-4.82-4-8a9 9 0 0 1 9-9z"/><line x1="9" y1="22" x2="15" y2="22"/>',
  },
  {
    name: '居住',
    color: '#AC725E',
    path: '<path d="M3 12L12 3l9 9"/><path d="M5 10v9a1 1 0 0 0 1 1h4v-5h4v5h4a1 1 0 0 0 1-1v-9"/>',
  },
  {
    name: '休閒',
    color: '#ABBC7C',
  ...
  },
  {
    name: '通訊',
    color: '#9785BC',
    path: '<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.56 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>',
  },
  {
    name: '其他',
    color: '#959595',
...
export function getUserCategories(): UserCategory[] {
  if (import.meta.client) {
    try {
      return JSON.parse(localStorage.getItem(USER_CATEGORIES_KEY) || '[]')
    }
    catch { return [] }
  }
  return []
...
export function saveUserCategory(cat: UserCategory) {
...
export function catColor(name: string): string {
  ...
  return builtinColorMap[name] ?? '#B0A090'
}

export function catPath(name: string): string {
  const user = getUserCategories().find(c => c.name === name)
  if (user) return LUCIDE_PATHS[user.icon] ?? DEFAULT_PATH
  return builtinPathMap[name] ?? DEFAULT_PATH
}
